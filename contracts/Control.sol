...
contract Control {
    ...
    function executeNextState(address _userAddress, uint _eventId, uint _functionParameter) public payable{
        ...
        bool callSucceedOrNot = mapFSMDetail[fsmVersion][nextState].contractAddress.call.value(msg.value)(method, _userAddress);
        if (callSucceedOrNot){ users[getUserIdx(_userAddress)].currentState = nextState; }
    }

    function userNextState(address _addr, uint _eventId) returns(uint, uint){
    ...
    uint public b;
    string public c;
    function executeNextStateUint(address _userAddress, uint _eventId, uint _functionParameter) public payable{
        uint fsmVersion;
        uint nextState;
        (nextState,fsmVersion) = userNextState(_userAddress, _eventId);
